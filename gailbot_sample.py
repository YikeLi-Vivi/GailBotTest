...
# @Last Modified time: 2024-04-07 10:23:18
from gailbot import GailBot
from gailbot import ProfileSetting

INPUT = "./input/hello2.wav"
OUTPUT = "./output/"
GOOGLE_API = (
    "../input/google-api.json"  # TODO: need to change with existing path to google api
)
...
def transcribe_with_whisper():
    ...
    if not gb.is_profile(whisper_profile_name):
        gb.create_profile(name=whisper_profile_name, setting=whisper_profile_setting)

    source_id = gb.add_source(INPUT, OUTPUT)
    gb.apply_profile_to_source(source_id=source_id, profile_name=whisper_profile_name)
...
def transcribe_with_watson():
    ...
    if not gb.is_profile(watson_profile_name):
        gb.create_profile(name=watson_profile_name, setting=watson_profile)
    source_id = gb.add_source(INPUT, OUTPUT)
    gb.apply_profile_to_source(source_id=source_id, profile_name=watson_profile_name)
